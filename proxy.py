...
def _open_log_file():
    ...
    filename = os.path.join(
        LOG_DIR, f"proxy.{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
...
def filter_beta_header(value: str) -> str | None:
    parts = [
        v.strip() for v in value.split(",") if v.strip() not in UNSUPPORTED_BETA_VALUES
    ]
...
async def proxy(request: Request, path: str):
    ...
    log(
        "INFO", f"→ {request.method} {url} {'[stream→sync]' if is_stream else '[sync]'}"
    )
    ...
    if is_stream:
        ...
        log(
            "INFO",
            f"  tokens: {response_body.get('usage', {})} | stop: {response_body.get('stop_reason')}",
        )
        return StreamingResponse(
            fake_sse_stream(response_body), media_type="text/event-stream"
        )
...
if __name__ == "__main__":
    ...
    log(
        "INFO",
        "Claude Code: set -a && source .env && set +a && claude --model <model_name>",
    )
